...
class ThemlerContentExporter {
    ...
    private function _getAbsoluteImagesData(&$content) {
        ...
        for ($i = 0; $i < $len; $i++) {
            ...
            $abs_path = FilesUtility::normalizePath($upload_dir . $relative_path);
            if (!is_file($abs_path)) {
                continue;
            }
    ...
    private function _getRelativeImagesData(&$content) {
        ...
        $len = count($matches[0]);
        for ($i = 0; $i < $len; $i++) {
            $path = FilesUtility::normalizePath($_SERVER['DOCUMENT_ROOT'] . $matches[1][$i]);
...
class ThemlerContentStorage {
    ...
    public function createFolder($json) {
        FilesUtility::emptyDir($this->_tmpDir);
        FilesUtility::createDir($this->_tmpDir);
        ...
        if (isset($json['Images'])) {
            ...
            FilesUtility::createDir($this->_tmpDir . '/images');
            foreach($images as $image) {
                $path = $image['_path'];
                FilesUtility::copyRecursive($path, $this->_tmpDir . '/images/' . $image['fileName']);
            }
        }
        FilesUtility::write($this->_tmpDir . '/content.json', json_encode($json, JSON_PRETTY_PRINT));
    }

    public function createZip($archive_name) {
        $archive_path = dirname($this->_tmpDir) . '/' . $archive_name;
        FilesUtility::createZip($this->_tmpDir, $archive_path);
    ...
    public function clear() {
        foreach($this->_clearFolders as $path) {
            FilesUtility::emptyDir($path, true);
        }
    }
}